...
**[CoCo]**
```
Crea un Cortex Agent in Snowflake Intelligence per Sanita e Life Sciences.
...
- Crea l'agente HEALTHCARE_COPILOT_AGENT connesso alla Semantic View HEALTHCARE_COPILOT.GOLD.SV_HEALTHCARE_COPILOT
  con modello di orchestrazione claude-4-sonnet, rispondendo sempre in italiano
  e contesto di business: organizzazione sanitaria in Italia (ospedale/clinica)
...
- "Quali pazienti hanno maggior rischio di riammissione urgente?"
- "Quali dipartimenti hanno il tasso di urgenze piu alto?"
- "Quante note cliniche richiedono follow-up immediato?"
- "Qual e l'importo totale delle richieste di rimborso in attesa?"
...
**[CoCo]**
```
Crea un'app Streamlit in Snowflake (SiS) nel database HEALTHCARE_COPILOT, schema AI, chiamata HEALTHCARE_APP.
L'app deve connettersi a Snowflake con la sessione attiva (get_active_session).
Includi 4 sezioni usando st.tabs:
1. KPI — card con st.metric con i totali e i ratio principali di PATIENT_360
2. Distribuzione — grafico a barre (st.bar_chart) per categoria/regione da CLINICAL_HEALTH
3. Top 20 — tabella interattiva (st.dataframe) ordinata per score di rischio da READMISSION_RISK
