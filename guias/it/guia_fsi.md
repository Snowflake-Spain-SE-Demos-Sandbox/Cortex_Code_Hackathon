...
**[CoCo]**
```
Crea un Cortex Agent in Snowflake Intelligence per Servizi Finanziari.
...
- Crea l'agente FSI_COPILOT_AGENT connesso alla Semantic View FSI_COPILOT.GOLD.SV_FSI_COPILOT
  con modello di orchestrazione claude-4-sonnet, rispondendo sempre in italiano
  e contesto di business: istituto finanziario in Italia (banca, investimenti, assicurazioni)
...
- "Quali clienti hanno il compliance score piu basso?"
- "Quanti alert di rischio critico sono aperti in questo momento?"
- "Qual e il volume totale di transazioni rifiutate questo mese?"
- "Quali clienti hanno il KYC scaduto o prossimo alla scadenza?"
...
**[CoCo]**
```
Crea un'app Streamlit in Snowflake (SiS) nel database FSI_COPILOT, schema AI, chiamata FSI_APP.
L'app deve connettersi a Snowflake con la sessione attiva (get_active_session).
Includi 4 sezioni usando st.tabs:
1. KPI — card con st.metric con i totali e i ratio principali di CLIENT_360
2. Distribuzione — grafico a barre (st.bar_chart) per categoria/regione da TRANSACTION_HEALTH
3. Top 20 — tabella interattiva (st.dataframe) ordinata per score di rischio da RISK_EXPOSURE
